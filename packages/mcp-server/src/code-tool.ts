...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { BELIEVE_API_KEY: requireValue(readEnv('BELIEVE_API_KEY') ?? client.apiKey, 'set BELIEVE_API_KEY environment variable or provide apiKey client option'), BELIEVE_BASE_URL: readEnv('BELIEVE_BASE_URL') ?? client.baseURL ?? undefined };
