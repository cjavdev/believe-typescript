...
export class BaseBelieve {
   ...
   */
  constructor({
    baseURL = readEnv('BELIEVE_BASE_URL'),
    apiKey = readEnv('BELIEVE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    const customHeadersEnv = readEnv('BELIEVE_CUSTOM_HEADERS');
    if (customHeadersEnv) {
...
Believe.Teams = Teams;
Believe.Matches = Matches;
Believe.Episodes = Episodes;
Believe.Quotes = Quotes;
Believe.BelieveResource = BelieveResource;
Believe.Conflicts = Conflicts;
Believe.Reframe = Reframe;
Believe.Press = Press;
Believe.Coaching = Coaching;
Believe.Biscuits = Biscuits;
